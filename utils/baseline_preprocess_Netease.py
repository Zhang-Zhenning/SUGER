...
user_item = load_obj(os.path.join(
    data_root, r"baseline_data\Netease\user_item_dict"))
user_bundle = load_obj(os.path.join(
    data_root, r"baseline_data\Netease\user_bundle_dict"))
bundle_item = load_obj(os.path.join(
    data_root, r"baseline_data\Netease\bundle_item_dict"))
        ...
        for bundle_pid in range(num_bundles):
            ...
            if subgraph[1] != []:
                subgraph_dict.append(
                    [AttNum_to_id("user", user_pid), AttNum_to_id("bundle", bundle_pid), subgraph[0], subgraph[1],
                     subgraph[2]])
